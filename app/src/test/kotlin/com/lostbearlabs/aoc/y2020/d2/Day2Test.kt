...
class Day2Test {
    @Test
    fun testSamplePart1() {
        val entries = parsePasswordEntries(sample)
        assertThat(numValidPasswords(entries) { validPart1(it) }).isEqualTo(2)
    }

    @Test
    fun part1() {
        val entries = parsePasswordEntries(input)
        assertThat(numValidPasswords(entries) { validPart1(it) }).isEqualTo(393)
    }

    @Test
    fun testSamplePart2() {
        val entries = parsePasswordEntries(sample)
        assertThat(numValidPasswords(entries) { validPart2(it) }).isEqualTo(1)
    }

    @Test
    fun part2() {
        val entries = parsePasswordEntries(input)
        assertThat(numValidPasswords(entries) { validPart2(it) }).isEqualTo(690)
    }
